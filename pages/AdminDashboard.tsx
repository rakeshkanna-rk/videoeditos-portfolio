...
const LIST_CONFIGS: Record<
  string,
  { prefix: string; fields: { key: string; label: string; type: string }[] }
> = {
  specializations: {
    prefix: "spec_",
    fields: [
  ...
  portfolio: {
    ...
    fields: [
      ...
      { key: "video", label: "Video URL / Upload", type: "video" },
    ],
  },
  reels: {
    prefix: "reel_",
  ...
  social_links: {
    prefix: "social_",
    fields: [
      { key: "name", label: "Social Network", type: "text" },
...
const MediaLibrary: React.FC<{
  ...
  const handleDeleteFile = async (name: string) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        showToast("File not found in storage. It may have already been deleted.", "info");
        await fetchFiles();
      }
    } catch (err: any) {
  ...
  const handleCopy = (name: string) => {
    const {
      data: { publicUrl },
    } = supabase.storage.from("content").getPublicUrl(`uploads/${name}`);
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h3 className="text-2xl font-black uppercase tracking-tighter text-white italic">
              Media <span className="text-sky-500">Vault</span>
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {files.map((file) => {
                ...
                return (
                  <div
                    key={file.id}
                    className="p-4 bg-slate-950 border border-slate-800 rounded-2xl group hover:border-sky-500/30 transition-all"
                  >
                    ...
                    <div className="mb-4">
                      <p
                        className="text-white font-bold text-xs truncate mb-1"
                        title={file.name}
                      >
                        {file.name}
                      </p>
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        <span className="text-slate-500">
          {TYPE_ICON[row.type] || TYPE_ICON.text}
        </span>
      ...
      <div className={`flex ${row.type === "video" ? "flex-col" : "gap-2 items-start"}`}>
        {isLong ? (
          <textarea
            value={value}
            onChange={(e) => onChange(row.id, row.key, e.target.value)}
            rows={2}
            className="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-sky-500/50 transition-colors resize-y"
          />
        ) : (
          <input
            type="text"
            value={value}
            onChange={(e) => onChange(row.id, row.key, e.target.value)}
            className="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-sky-500/50 transition-colors"
          />
        )}
        {isImage && (
          ...
          <div className="flex flex-col gap-2 mt-2 w-full">
            <div className="flex gap-2">
              <button
                onClick={() => fileInputRef.current?.click()}
                disabled={uploading}
                className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors disabled:opacity-50 text-[10px] font-bold uppercase"
      ...
      {showPicker && (
        <IconPicker
          onSelect={(name) => onChange(row.id, row.key, name)}
...
const LIST_FIELD_MAPS: Record<string, Record<string, string>> = {
  ...
  social_links: { name: "name", url: "url", icon: "icon" },
};
...
export const AdminDashboard: React.FC = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [reorderTab, setReorderTab] = useState<string | null>(null);
  ...
  const fetchAllContent = async () => {
    try {
      ...
      const { data, error: fetchError } =
        await supabase.rpc("get_site_content");
  ...
  const handleFieldChange = (id: string, key: string, value: string) => {
    setAllContent((prev) =>
      prev.map((row) =>
        row.id === id && row.key === key ? { ...row, value: value } : row,
      ),
    );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const modifiedRows = allContent.filter((r) =>
        modifiedIds.has(`${r.id}:${r.key}`),
      );
  ...
  const handleAddListItem = async (sectionName: string) => {
    ...
    const insertData: any = {
      sort_order:
        allContent.filter((r) => r.section_name === sectionName).length * 10,
    };
  ...
  const handleOpenReorder = (section: string) => {
    const sectionData = grouped[section] || [];
    const ids = Array.from(new Set(sectionData.map((r) => r.id)));
    const items = ids.map((id) => {
      const row = sectionData.find(
        (r) =>
          r.id === id &&
          (r.key.endsWith("_title") ||
            r.key.endsWith("_name") ||
            r.key.endsWith("_role")),
      );
      return {
        id,
        label: row?.value || "Untitled Item",
      };
    });
    setReorderItems(items);
    setReorderTab(section);
  };

  const handleSaveOrder = async () => {
    if (!reorderTab) return;
    setSaving(true);
    try {
      const tableName = TABLE_MAPPING[reorderTab];
  ...
  const itemIds = isListSection
    ? Array.from(new Set(activeTabData.map((r) => r.id)))
    : [];

  // Special case for Contact tab to include Social Links
  const socialLinksData =
    activeTab === "contact" ? grouped["social_links"] || [] : [];
  const socialLinksIds =
    activeTab === "contact"
      ? Array.from(new Set(socialLinksData.map((r) => r.id)))
      : [];
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          {TAB_GROUPS[activeMode].map((tab) => (
            ...
            >
              ...
              {activeTab === tab.id && (
                <div
                  className={`w-1 h-4 rounded-full ${activeMode === "editor" ? "bg-sky-500" : "bg-purple-500"}`}
                />
              )}
            </button>
          ))}
        ...
        <header className="flex justify-between items-end mb-12">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span
                className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${activeMode === "editor" ? "bg-sky-500/20 text-sky-400 border border-sky-500/30" : "bg-purple-500/20 text-purple-400 border border-purple-500/30"}`}
              >
                {activeMode} mode
              </span>
              <span className="text-slate-700 font-mono text-[10px] uppercase">
                {activeTab}
              </span>
            </div>
            <h2 className="text-5xl font-black uppercase tracking-tighter italic">
              {SECTION_LABELS[activeTab]?.split(" ").slice(1).join(" ") ||
                activeTab}
            </h2>
          </div>
        ...
        <div className="max-w-5xl">
          {loading && allContent.length === 0 ? (
            <div className="flex items-center justify-center py-32">
              <RefreshCw
                size={48}
                className="text-sky-500 animate-spin opacity-20"
              />
            </div>
          ) : error ? (
                ...
                <div className="space-y-12">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => handleOpenReorder(activeTab)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeMode === "editor" ? "bg-slate-800 text-slate-400 hover:text-white border border-slate-700" : "bg-slate-800 text-slate-400 hover:text-white border border-slate-700"}`}
                      >
                          ...
                          <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                              ...
                              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                                {LIST_CONFIGS[activeTab].fields[0].label}:{" "}
                                {itemRows.find(
                                  (r) =>
                                    r.key.endsWith("_title") ||
                                    r.key.endsWith("_name") ||
                            ...
                            <button
                              onClick={() =>
                                handleDeleteListItem(activeTab, id)
                              }
                            ...
                            {itemRows
                              ...
                              .map((row) => (
                                <ContentField
                                  ...
                                  modified={modifiedIds.has(
                                    `${row.id}:${row.key}`,
                                  )}
                                  uploading={
                                    uploadingId === `${row.id}:${row.key}`
                                  }
                                />
                              ))}
                          </div>
                  ...
                  {activeTab === "contact" ? (
                    <div className="space-y-12">
                      ...
                      <div className="space-y-6">
                        <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 border-l-2 border-sky-500 pl-4">
                          Header Content
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {activeTabData
                            .filter((r) =>
                              ["heading", "description"].includes(r.key),
                            )
                            .map((row) => (
                              <ContentField
                                key={row.id + row.key}
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {activeTabData
                            .filter((r) => ["email", "phone"].includes(r.key))
                            .map((row) => (
                              <ContentField
                                key={row.id + row.key}
                                row={row}
                      ...
                      {/* 3. Social Connections (Moved here) */}
                      <div className="space-y-8">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => handleOpenReorder("social_links")}
                              className="flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest bg-slate-800 text-slate-400 hover:text-white border border-slate-700 transition-all"
                            >
                              <GripVertical size={14} />
                              Reorder
                            </button>
                            <button
                              onClick={() => handleAddListItem("social_links")}
                              className="flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 border border-sky-500/20 transition-all"
                            >
                              <Plus size={14} />
                              Add Social Link
                            </button>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          {socialLinksIds.map((id, i) => {
                            const itemRows = socialLinksData.filter(
                              (r) => r.id === id,
                            );
                            return (
                              <motion.div
                                ...
                                <div className="flex items-center justify-between mb-6">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                                      {itemRows.find((r) =>
                                        r.key.endsWith("_name"),
                                      )?.value || "New Social Link"}
                                    </h4>
                                  </div>
                                  <button
                                    onClick={() =>
                                      handleDeleteListItem("social_links", id)
                                    }
                                    className="p-2 text-slate-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"
                                  >
                                    <Trash2 size={16} />
                          ...
                          {socialLinksIds.length === 0 && (
                            <div className="py-12 text-center border-2 border-dashed border-slate-800/30 rounded-3xl">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {activeTabData
                            .filter((r) => r.key.startsWith("contact_"))
                            .map((row) => (
                              <ContentField
                                key={row.id + row.key}
                                row={row}
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
            ...
            >
              <div className="p-8 border-b border-slate-800 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black uppercase tracking-tighter italic text-sky-400">
                    Reorder Items
                  </h2>
                ...
                >
                  {reorderItems.map((item) => (
                    <Reorder.Item
                      key={item.id}
                      value={item}
                      className="p-4 bg-slate-950 border border-slate-800 rounded-2xl flex items-center gap-4 group cursor-grab active:cursor-grabbing hover:border-sky-500/30 transition-colors"
                    >
                      <GripVertical
                        className="text-slate-700 group-hover:text-sky-500 transition-colors"
                        size={18}
                      />
              ...
              <div className="p-8 bg-slate-950/50 flex gap-4">
                <button
                  onClick={() => setReorderTab(null)}
                  className="flex-1 py-4 bg-slate-900 text-slate-400 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-slate-800 transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={saving}
                  className="flex-1 py-4 bg-sky-500 text-slate-950 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-sky-400 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {saving ? (
                    <RefreshCw size={14} className="animate-spin" />
                  ) : (
